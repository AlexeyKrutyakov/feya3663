...
- **README is partly aspirational.** `apps/web` is not committed to `main` yet —
  references in `README.md` describe target state. `docker-compose.yml`
  (postgres + redis) and `.env.example` exist; copy `.env.example` to `.env`
  and run `docker compose up -d` for local Postgres/Redis.
