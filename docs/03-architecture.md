...
flowchart LR
    ...
    MIS -->|webhook: visit create/update| API
    API -->|SMS| SMS[SMS-провайдер]
    API -.->|после MVP, через VPS-прокси| TG[Telegram Bot API]
```
...
Notification    — наше уведомление
  id, userId, appointmentId, channel (sms|email), type (reminder|change|cancel),
...
| Уведомления (MVP) | SMS (рос. провайдер); Telegram — после MVP (VPS-прокси) | ✅ согласовано |
...
- **Провайдер SMS/email** — для уведомлений (выбор — Q8).
...
- **Синхронизация (чтение):** вебхуки (основной путь) + периодическая дельта-сверка через параметр `modificate` (только изменённые `visit`/`employee`); идемпотентность по `misVisitId` + `update_date`.
...
- **Часовые пояса:** нормализация `datetime` (приходит и со смещением, и без).
- **Безопасность:** TLS, шифрование чувствительных полей, аудит доступа к медданным, защита токена MIS, проверка подписи вебхуков (если поддерживается).
...
- **Решение:** проекция (кэш) + вебхуки как основной канал чтения + периодический pull для сверки; запись — синхронные вызовы MIS из очереди с повторами, подтверждение по вебхуку.
- **Последствия:** возможны кратковременные расхождения (закрываются сверкой) и гонки чтение/запись; нужна идемпотентность и аккуратная обработка ошибок MIS.
...
### ADR-004: Доставка Telegram-уведомлений в РФ
- **Статус:** 🟡 отложено (после MVP)
- **Контекст:** Telegram Bot API недоступен из РФ напрямую. В MVP канал уведомлений — только SMS.
- **Решение (на будущее):** исходящие запросы к Bot API через прокси на отдельном VPS вне РФ.
- **Последствия:** доп. инфраструктура; для MVP не делаем — SMS достаточно.
